/*
  # Fix Renter Lease Signing Trigger
  
  ## Overview
  Updates the renter journey tracker to properly handle lease signing.
  The lease_signed field and lease_signing stage should only be set when
  the rental application status is actually 'lease_signed', not when 'approved'.
  
  ## Changes
  - Update trigger to properly set lease_signed field when status is 'lease_signed'
  - Ensure stage progression only moves to lease_signing after approval
  - Add proper date tracking for when lease is actually signed
*/

-- Update the trigger to properly handle lease signing
