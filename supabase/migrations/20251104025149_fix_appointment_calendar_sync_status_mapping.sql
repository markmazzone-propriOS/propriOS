/*
  # Fix Appointment Calendar Sync Status Mapping
  
  1. Changes
    - Map 'scheduled' status to 'pending' when syncing to calendar_events
    - service_provider_appointments uses: scheduled, confirmed, cancelled, completed
    - calendar_events uses: pending, confirmed, cancelled, completed
  
  2. Notes
    - Ensures status values are compatible between the two tables
*/
...
BEGIN
  -- Map 'scheduled' to 'pending' for calendar_events compatibility
