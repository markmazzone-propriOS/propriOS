/*
  # Fix Agent Notification Query

  ## Overview
  The agent_profiles table uses a `locations` array field, not individual
  city and state columns. This migration fixes the query to work with the
  actual schema.

  ## Changes
  - Update query to check if property location matches agent's locations array
  - Fix ORDER BY to work with array field
*/
...
BEGIN
  ...
  v_property_location := NEW.city || ', ' || NEW.state;

  -- Find agents who service this location
  FOR v_agent IN
    SELECT DISTINCT a.id, p.full_name, u.email, a.locations
    ...
    AND (
      a.locations @> ARRAY[v_property_location]::text[]
    ...
    IF v_service_role_key IS NOT NULL AND v_agent.email IS NOT NULL THEN
      BEGIN
        ...
        IF v_response_status >= 200 AND v_response_status < 300 THEN
          ...
          RAISE NOTICE 'Email sent to agent %', v_agent.email;
        ELSE
  ...
  ELSE
    RAISE NOTICE 'No agents were emailed for property at %', NEW.address_line1;
  END IF;
