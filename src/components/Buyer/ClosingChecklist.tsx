...
export function ClosingChecklist() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('closing-checklist-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'closing_checklist_items',
  ...
  const generateDefaultChecklist = async () => {
    ...
    try {
      const { error } = await supabase.rpc('create_default_closing_checklist', {
        p_buyer_id: user.id,
        p_property_id: null
      });
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
          ...
          <p className="text-gray-600 mb-6 max-w-md mx-auto">
            Generate a comprehensive closing checklist to help you track all the important tasks before closing on your property.
          </p>
          <button
...
function AddItemModal({ onClose, onItemAdded }: AddItemModalProps) {
  const { user } = useAuth();
