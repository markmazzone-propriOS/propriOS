...
import { Search, ArrowLeft, Map, Grid, SlidersHorizontal, X } from 'lucide-react';
...
export function RentPage() {
  ...
  const handleViewModeChange = (mode: 'grid' | 'map') => {
    setViewMode(mode);
    sessionStorage.setItem('rentPageViewMode', mode);
  };

  const handlePropertyClick = (propertyId: string) => {
    sessionStorage.setItem('rentPageViewMode', viewMode);
    navigate(`/properties/${propertyId}?from=rent`);
  };
    ...
    <div>
      <div className="bg-white shadow-sm border-b mb-8">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Rent a Home</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {showFilters && (
          <div className="mb-6 bg-white rounded-lg border border-gray-300 p-6 shadow-sm">
            ...
            <div className="mt-4 text-sm text-gray-600">
              Showing <span className="font-semibold text-gray-800">{filteredProperties.length}</span> of <span className="font-semibold text-gray-800">{properties.length}</span> homes for rent
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">No homes for rent found</p>
          </div>
        ) : viewMode === 'grid' ? (
