...
import { Plus, Search, ArrowLeft, Map, Grid, SlidersHorizontal, X } from 'lucide-react';
...
export function PropertyList() {
  ...
  const handleViewModeChange = (mode: 'grid' | 'map') => {
    setViewMode(mode);
    sessionStorage.setItem('propertyListViewMode', mode);
  };

  const handlePropertyClick = (propertyId: string) => {
    sessionStorage.setItem('propertyListViewMode', viewMode);
    navigate(`/properties/${propertyId}?from=list`);
  };
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    const matchesType = listingType === 'all' || property.listing_type === listingType;
    ...
    <div>
      <div className="bg-white shadow-sm border-b mb-8">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            {user && (profile?.user_type === 'seller' || profile?.user_type === 'agent' || profile?.user_type === 'service_provider') && (
              <button
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-6 flex flex-col md:flex-row gap-4">
          ...
          <select
            value={listingType}
            onChange={(e) => setListingType(e.target.value as 'all' | 'sale' | 'rent')}
        ...
        {showFilters && (
          <div className="mb-6 bg-white rounded-lg border border-gray-300 p-6 shadow-sm">
            ...
            <div className="mt-4 text-sm text-gray-600">
              Showing <span className="font-semibold text-gray-800">{filteredProperties.length}</span> of <span className="font-semibold text-gray-800">{properties.length}</span> properties
            </div>
          </div>
        )}
